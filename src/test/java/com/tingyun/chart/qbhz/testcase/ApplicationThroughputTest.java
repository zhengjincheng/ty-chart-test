...
public class ApplicationThroughputTest extends TingyunChartTestCase {
	...
	public void queryByEndtimeAndTimePeriod(String endtime, int timePeriod) throws SQLException {
		...
		ChartTestInput input = ChartTestInput.build().userName("sina").passWord("1").chartid("application-throughput")
		...
		// 获取sql的数据
		String sql = "select    round(round(sum(count)/$tmTick,4),3) as count_throughtout,sum(count) as count,$sql_tmTick  from NL_APP_APPLICATION_OVERVIEW$table_postfix	 where  timestamp >= $sql_begintime AND timestamp < $sql_endtime and application_instance_id in ( $instanceId)  and count > 0	 group by tmTick order by tmTick asc";
