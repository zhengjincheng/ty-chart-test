...
public class JvmSessionTest extends TingyunChartTestCase {
	...
	private String context_path_id=JvmTestConst.context_path_id;
	...
	public void queryByEndtimeAndTimePeriod(String endtime, int timePeriod) throws SQLException {
		...
		ChartTestInput input = ChartTestInput.build().userName("sina").passWord("1").chartid("jvm-session")
				.put("applicationId", applicationId).put("context_path_id", context_path_id).put("vm_id", vm_id).put("timePeriod", String.valueOf(timePeriod))
		...
		while (rs.next()) {
			...
			Assert.assertEquals(String.valueOf(rs.getInt(1)), b.getSeries().get(0).getData().get(i).getTooltipObject().getData().get(0).getValue());
//			Assert.assertEquals(String.valueOf(rs.getInt(4)), b.getSeries().get(0).getData().get(i).getTooltipObject().getData().get(2).getValue());
