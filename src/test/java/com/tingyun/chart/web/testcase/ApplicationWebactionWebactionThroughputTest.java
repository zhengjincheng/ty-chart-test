...
public class ApplicationWebactionWebactionThroughputTest extends TingyunChartTestCase {
	...
	public void queryByEndtimeAndTimePeriod(String endtime, int timePeriod) throws SQLException {
		...
		ChartTestInput input = ChartTestInput.build().userName("sina").passWord("1")
				.chartid("application-webaction-webaction-throughput").put("applicationId", applicationId)
		...
		// 获取sql的数据
		String sql = "select   sum(count)/$tmTick as count_throughtout,sum(count) as count,sum(duration_total)/sum(count)/1000 as duration_total,max(duration_max)/1000 as duration_max,min(duration_min)/1000 as duration_min,sqrt((sum(duration_squaresum) - sum(duration_total)  * sum(duration_total) / sum(count))/(sum(count)-1))/1000 as duration_square,$sql_tmTick from NL_APP_WEB_ACTION$table_postfix	 where  timestamp >= $sql_begintime AND timestamp < $sql_endtime and application_instance_id in ( $instanceId)  AND count > 0	 AND action_id in ( $actionId )  group by tmTick order by tmTick asc";
