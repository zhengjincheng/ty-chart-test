...
public class GoMemoryStackSysInuseTest extends TingyunChartTestCase {
	...
	public void queryByEndtimeAndTimePeriod(String endtime, int timePeriod) throws SQLException {
		...
		ChartTestInput input = ChartTestInput.build().userName("sina").passWord("1").chartid("go-memory-stack-sys-inuse")
		...
		// 获取sql的数据
		String sql = "select    round(sum(stack_sys_total)/sum(count),1) as countTotal,sum(stack_sys_max)/sum(count) as countMax,sum(stack_sys_min)/sum(count) as countMin, FORMAT(sum(stack_inuse_total)/sum(count),1) as countTotalTo,sum(stack_inuse_max)/sum(count) as countMaxTo,sum(stack_inuse_min)/sum(count) as countMinTo,$sql_tmTick  from NL_VM_GO_MEMORY$table_postfix	 where  timestamp >= $sql_begintime AND timestamp < $sql_endtime and vm_id = $vm_id and count > 0	 group by tmTick order by tmTick asc";
