...
public class ApplicationGoRuntimeLookupsTest extends TingyunChartTestCase {
	...
	public void queryByEndtimeAndTimePeriod(String endtime, int timePeriod) throws SQLException {
		...
		ChartTestInput input = ChartTestInput.build().userName("sina").passWord("1")
				.chartid("application-go-runtime-lookups").put("applicationId", "127050").put("vm_id", "424")
		...
		// 获取sql的数据 round(,1)
		String sql = "select  round( sum(lookups_total)/$tmTick,1) as countTotal,$sql_tmTick  from NL_VM_GO_RUNTIME$table_postfix	 where  timestamp >= $sql_begintime AND timestamp < $sql_endtime and vm_id = 424 and count > 0	 group by tmTick order by tmTick asc";
