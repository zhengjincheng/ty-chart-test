...
//	}
	...
	public void queryByEndtimeAndTimePeriod(String endtime, int timePeriod) throws SQLException {
		...
		// 获取sql的数据
		String sql = "select sum(gc_time_total) as gc_time,$sql_tmTick  from NL_VM_GO_GC$table_postfix	 where  timestamp >= $sql_begintime AND timestamp < $sql_endtime and vm_id = $vm_id and count > 0	 group by tmTick order by tmTick asc";
